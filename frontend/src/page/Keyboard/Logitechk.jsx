...
function Logitechk() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await request1.get("goods/9/getByBrandId");
    ...
    <div className="test">
      <div className="flex divide-y divide-gray-100 rounded-lg my-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3 relative gap-y-10 gap-x-10 basis-[80%] ml-4">
            <div className="text-center my-10 text-3xl font-semibold font-Montserrat hover:text-primary transition-all duration-300 cursor-pointer items-center my-60">
                Bàn phím Microsoft chính hãng
            </div>
          {products &&
            products.length > 0 &&
